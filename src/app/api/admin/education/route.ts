import { getEducationResolved, writeEducationFile } from "@/lib/content";
...
export async function GET() {
  ...
  return Response.json(await getEducationResolved());
}
  ...
  if (process.env.VERCEL === "1") {
    ...
    if (!db) {
      return Response.json(
        {
          error:
            "Vercel cannot write content/education.json. Add DATABASE_URL or POSTGRES_URL (Neon) in Vercel → Environment Variables for Production, redeploy, then run pnpm db:migrate so the cms_json_docs table exists.",
        },
        { status: 503 },
      );
    }
    try {
      await upsertCmsJsonDoc("education", parsed.data);
    } catch (e) {
